 ...
 * @author Gili Tzabari
 */
class ArrayVerifierImpl<E> implements ArrayVerifier<E>
{
	/**
	 * @param <E>   the type of elements in the array
	 ...
	 */
	ArrayVerifierImpl(SingletonScope scope, E[] actual, String name, Configuration config)
	{
		...
		this.name = name;
		this.config = config;
		this.asCollection = new CollectionVerifierImpl<>(scope, asList(actual), name, config,
			Pluralizer.ELEMENT);
	}
