...
public final class DiffGenerator
{
	...
	public DiffResult diff(String actual, String expected)
	{
		...
		if (DiffConstants.NEWLINE_PATTERN.matcher(actual).find() || DiffConstants.NEWLINE_PATTERN.matcher(expected).find())
		{
			// If the input contains multiple lines, add the end of string character
			actualWithEos = actual + DiffConstants.EOS_MARKER;
			expectedWithEos = expected + DiffConstants.EOS_MARKER;
		}
		else
		{
