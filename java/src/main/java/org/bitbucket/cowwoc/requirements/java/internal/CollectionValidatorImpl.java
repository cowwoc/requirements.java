	...
	implements CollectionValidator<C, E>
{
	 ...
	 * @throws AssertionError if {@code scope}, {@code name}, {@code pluralizer}, {@code config} or
	 *                        {@code failures} are null. If {@code name} is empty.
	 */
	public CollectionValidatorImpl(ApplicationScope scope, String name, C actual, Pluralizer pluralizer,
	...
	{
		return new SizeValidatorImpl(scope, name, actual, name + ".size()", actual.size(), pluralizer,
			config, failures);
	}
	...
	public ArrayValidator<E> asArray(Class<E> type)
	{
		...
		return new ArrayValidatorImpl<>(scope, name, actual.toArray(array), config, failures);
	}
