...
final class Doubles<S>
{
	...
	private S isEqualToImpl(double expected, MaybeUndefined<String> name)
	{
	...
	private S isNotEqualToImpl(double unwanted, MaybeUndefined<String> name)
	{
	 ...
	 *                                  </ul>
	 */
	public S isBetween(double minimum, boolean minimumInclusive, double maximum, boolean maximumInclusive)
	{
		return isBetween((Double) minimum, minimumInclusive, (Double) maximum, maximumInclusive);
	}
	...
	private S isMultipleOfImpl(double factor, MaybeUndefined<String> name)
	{
		if (validator.value.isNull())
			validator.onNull();
	...
	private S isNotMultipleOfImpl(double factor, MaybeUndefined<String> name)
	{
		if (validator.value.isNull())
			validator.onNull();
		switch (validator.value.test(value -> !Numbers.isMultipleOf(value, factor)))
